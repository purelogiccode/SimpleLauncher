...
public class SupermodelConfigHandler : IEmulatorConfigHandler
{
    ...
    {
        return emulatorName.Contains("Supermodel", StringComparison.OrdinalIgnoreCase) ||
               (emulatorPath?.Contains("Supermodel.exe", StringComparison.OrdinalIgnoreCase) ?? false);
    }
        ...
        if (context.EmulatorManager != null)
        {
            ...
            else
            {
                ...
                if (File.Exists(resolvedExe))
                    SupermodelConfigurationService.InjectSettings(resolvedExe, context.Settings!, _logger);
            }
