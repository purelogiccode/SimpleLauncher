...
# to 3 (Unix). The central directory is walked record by record from the offset in the
# end-of-central-directory record — patching every PK\x01\x02 found in the file would corrupt
# an entry whose compressed payload happens to contain those bytes.
function Set-UnixHostSystem {
    param([Parameter(Mandatory = $true)][string]$ZipPath)

    $bytes = [System.IO.File]::ReadAllBytes($ZipPath)

    # Find the end-of-central-directory record (PK\x05\x06); the trailing comment is at most
    # 64 KiB long, so the search is bounded to that window.
    $eocdOffset = -1
    $minOffset = [Math]::Max(0, $bytes.Length - 22 - 65535)
    for ($i = $bytes.Length - 22; $i -ge $minOffset; $i--) {
    ...
    if ($eocdOffset -lt 0) {
        throw "Could not find the end-of-central-directory record in '$ZipPath'."
    }

    $directoryOffset = [System.BitConverter]::ToUInt32($bytes, $eocdOffset + 16)
    if ($directoryOffset -eq 0xFFFFFFFF) {
        throw "ZIP64 archives are not supported by the Unix-mode patcher ('$ZipPath')."
    }

    $patched = 0
    $index = [int]$directoryOffset
    while ($index + 46 -le $bytes.Length -and
           $bytes[$index] -eq 0x50 -and $bytes[$index + 1] -eq 0x4B -and
           $bytes[$index + 2] -eq 0x01 -and $bytes[$index + 3] -eq 0x02) {
        if ($bytes[$index + 5] -ne 0x03) {
            $bytes[$index + 5] = 0x03
            $patched++
        }

        # Advance to the next central-directory record: fixed 46-byte header plus the
        # variable-length file name, extra field and comment sections.
        $fileNameLength = [System.BitConverter]::ToUInt16($bytes, $index + 28)
        $extraFieldLength = [System.BitConverter]::ToUInt16($bytes, $index + 30)
